...
export const fetchTrasporti = async (
  token: string
): Promise<TrasportoInterface[]> => {
...
export const createTrasporto = async (
  data: TrasportoInterface,
  token: string
): Promise<TrasportoInterface> => {
  console.log("URL completo:", `${LOCAL_HOST}/trasporti`);
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Errore nella creazione del trasporto: ${response.status} ${response.statusText}`
      );
    }
...
export const updateTrasporto = async (
  id: string,
  data: TrasportoInterface,
  token: string
): Promise<TrasportoInterface> => {
...
export const deleteTrasporto = async (
  id: string,
  token: string
): Promise<void> => {
  const response = await fetch(`${LOCAL_HOST}/trasporti/${id}`, {
